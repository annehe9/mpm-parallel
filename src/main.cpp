...
void P2G_iteration(Particle& p) {
    ...
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 3; j++) {
            ...
            tmp = affine * dpos;
	        Vector3d accum = w[i].x() * w[j].y() * (mass_x_velocity + Vector3d(tmp.x(), tmp.y(), 0));
            p.grid_update[i * 3 + j] = accum;
	        //#pragma omp critical
            //grid[base_coord.x() + i][base_coord.y() + j] += accum;
        }
    }
}
...
void ResetAssignment(void)
{
    ...
    for (int i = 0; i < NUM_PARTICLES; i++) {
        ...
        int count = 0;
        while (assignment[x * GRID_RES + y][count] != -1) {
            count++;
        }
        // save particle index
        assignment[x * GRID_RES + y][count] = i;
    }
}
...
void P2GGrid_iteration(Vector3d& g, int i, int j) {
    ...
    for (int x = 0; x < 3; x++) {
        for (int y = 0; y < 3; y++) {
            for (int k = 0; k < MAX_PARTICLES_PER_CELL; k++) {
                ...
                int offset_x = (x - 1);
                int offset_y = (y - 1);
                int pdx = assignment[(i + offset_x) * GRID_RES + (j + offset_y)][k];
                if (pdx == -1) break;
                ...
                offset_x += 1;
                offset_y += 1;
                g += particles[pdx].grid_update[offset_x * 3 + offset_y];
            }
        }
    }
}
